...
function SavedCount() {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = () => {
      const updated = JSON.parse(localStorage.getItem('savedUniversities') || '[]');
            ...
            {countries.map((country) => (
              <Link
                ...
                className={`block w-full text-left px-4 py-2 text-sm flex items-center gap-2 rounded-md ${
                  selectedCountry === country.code ? 'bg-primary/10 text-primary' : 'hover:bg-muted/50'
                }`}
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <Link href="/" className="flex items-center gap-2 group">
            ...
            <span className="font-bold text-foreground hidden sm:inline text-lg tracking-tight">StudyConnect</span>
          </Link>
        ...
        {isOpen && (
          <nav className="md:hidden pb-4 flex flex-col gap-4 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div className="py-2 -mx-3 px-3">
              <div className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Countries</div>
            ...
            <div className="flex gap-3 pt-2">
              <Link href="/login" className="flex-1 px-4 py-2 text-foreground/70 hover:text-primary transition font-medium text-sm border border-border/40 rounded-lg text-center">
                Sign In
              </Link>
              <Link href="/signup" className="flex-1 px-4 py-2 gradient-primary text-primary-foreground rounded-lg hover:shadow-lg transition font-semibold text-sm text-center">
                Get Started
              </Link>
            </div>
          </nav>
